...
- The staging bot and protected deployment workflow currently derive a domain-separated 32-byte launch-signing key from the staging Discord bot token. Cloudflare receives only that derived key as `LAUNCH_SIGNING_KEY_HEX`.
- Token-derived signing is explicit staging bootstrap debt. The long-term hardening path is one coordinated cutover to a dedicated random launch-signing secret shared only by Bloom and the protected Cloudflare deployment environment.
...
The permanent staging workflow lives at `.github/workflows/moderation-web-staging-deploy.yml` in this repository. It runs the build/tests and Wrangler dry run, deploys to Cloudflare, then proves health/origin, unauthenticated rejection, signed first use, authenticated session page access, and exact-link replay rejection on the permanent staging hostname.
