...
All permissions default to **OP** unless noted otherwise. Permissions are
...
Permission nodes matching `enthusiamarket.limit.<group-name>` grant membership
in a named limit group from config. Effective limits merge by taking the best
value per dimension across all granted groups.
...
Players with no limit group get `defaultStallLimit` (config, default -1 =
unlimited).
