...
summary:
  Infrastructure adapters — persistence, commands, listeners, DI wiring, and
  integrations.
...
The infrastructure layer (`infrastructure/`) contains all framework adapters. It
can import anything.
...
All use **prepared statements** (no raw SQL concatenation). SQL is portable
between SQLite and MariaDB — tested via `SqlPortabilityTest`.
...
7. Starts schedulers (rent collection, auction settlement, sign refresh, shop
   audit)
