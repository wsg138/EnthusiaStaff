...
topic: application
summary:
  Application services and use cases — what happens between domain and
  infrastructure.
...
The application layer (`application/`) implements use cases. It depends on
domain ports only — no framework code.
...
Every mutation (upsert, delete, etc.) updates both the DB and the in-memory
index. `findByContainer` serves directly from the index — zero DB queries on the
hopper server thread.

### Constructor injection

All services receive their dependencies via constructor parameters. DI wiring
happens in `EnthusiaMarket.onEnable`:
...
Services return sealed `Result` classes (not exceptions) for predictable control
flow:
