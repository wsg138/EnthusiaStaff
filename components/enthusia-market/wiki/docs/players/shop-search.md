...
Type any Minecraft item name. Tab-complete is supported — press Tab to see
material names matching your typed prefix (case-insensitive).
...
New shops are **searchable by default**. Shop owners can toggle this in the edit
GUI (`/shop edit`). The server can change the default via the
`shop.search.default` config key.

## Tips

- **Sell search** means you're looking to BUY from shops that SELL. You're the
  customer.
- **Buy search** means you're looking to SELL to shops that BUY. You're the
  supplier.
