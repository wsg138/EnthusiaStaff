...
topic: shop-creation
summary:
  "How to create BUY, SELL, and TRADE shops — shift-click for the GUI or place a
  sign with text."
...
Shops are the core of EnthusiaMarket. You link a container to a wall sign inside
your stall, configure what you trade and at what price, and you're in business.

There are two ways to create a shop — the GUI method (recommended) and the
text-sign method (fallback).

## Method 1: GUI (recommended)

This method opens a menu where you pick the trade direction, amounts, and
payment item without typing anything on the sign.

### Step-by-step

1. **Place a blank wall sign** on a chest, barrel, shulker box, or other
   container **inside your stall**.
...
6. **Set your price** — currency amount for BUY/SELL, or hold the payment item
   for TRADE.
7. **Confirm** — the plugin creates the shop, writes the sign text, and links
   everything.

> **Bedrock players:** A form opens instead of the Java GUI. The process is the
> same — pick trade direction, amounts, and confirm.

## Method 2: Text sign (fallback)

The classic sign-based method from earlier versions. Write the trade details
...
4. **Confirm the sign.** The plugin validates your input, creates the shop, and
   reformats the sign text.
