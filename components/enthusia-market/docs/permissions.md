...
Permission node taxonomy. All nodes are namespaced under `enthusiamarket.*` with
`em.*` as alias. Nodes marked **(default: op)** require LuckPerms / operator
status by default; all others default to `true` (granted).
