...
Status: `ACTIVE` / `ACTIONABLE_CONTINUATION` — implementation complete; synchronized final-review corrections applied; fresh exact-head validation pending.
...
- The old private Pi attempt reached `Lincoln-PI-4` but failed before Paper because a transient bridge artifact returned HTTP 404. That is non-passing evidence.
- Current target `main` `2d8fcf27b0bac980211149ae8f7f4e7798998ee5` was then normally merged into the package branch as `562e8647063c3fa09b4349d167dc41d1a1660553`, restoring PR mergeability without changing ES-P07 product files.
...
- Final review now additionally requires both permission nodes to contain an explicit non-null `default` field before asserting their default value is false.
- Inventory safety documentation is updated without claiming deferred representative broader private acceptance.
...
- Synchronized candidate `562e8647063c3fa09b4349d167dc41d1a1660553` passed Java 21 clean/full tests with MariaDB/Testcontainers, runtime-JAR/provider-leak inspection, aggregate JaCoCo/Codacy upload, Wiki and exact Sentinel artifact production. CodeRabbit then found two valid final-review issues: a missing explicit metadata-field-presence assertion and stale tracked freeze-state wording.
- This checkpoint fixes both. Therefore `562e864...` and all earlier heads are superseded for final acceptance, including their hosted successes and queued runtime work.

## Superseded runtime evidence

- Exact synchronized-head Sentinel artifact run `31431731862` succeeded and produced artifact `9079413472`, digest `sha256:3d0dff42e624891030ab1af6f19d7a6cd82c58b13c62da2603d44ad958034ff3`.
- Sentinel restart job 83 for `562e864...` was accepted into the durable queue while the shared Pi reported 132 MB free memory and 82.8 C. It is not a pass and becomes stale for final acceptance after this review-fix commit.
- Exact synchronized-head Coverage run `31431731890` / job `93596510081` succeeded with aggregate 47.14% line, 38.24% branch and 49.80% instruction coverage; Paper JAR 9,148,983 bytes SHA-256 `7de4853ecc8d005a6277178284936a51988dee161991df8cbd6ba8649b22f8fa`; Velocity JAR 7,891,242 bytes SHA-256 `cefb70557829777b4c9fff3a78bf6a14731fa347fa6f3f60fd451a04e661c19a`; provider API leaks zero; validation artifact `9079582840`, digest `sha256:2d4d04410a34473f951577fbf0a4b8bacc14b81ca0c74b4744d0d284c04aff1a`. These are diagnostic only after the review correction.
- Any public/private Pi run for `562e864...` is likewise superseded and must not be used as final proof.
