...
| Active/selected package | `ES-V01 — Private LiteBans representative-data verification`; `PARTIAL` / `ACTIONABLE_CONTINUATION` on `package/es-v01-litebans-private-verification`. |
| Highest-priority next continuation | ES-V01 remains active until its hosted exact-head validation, review, PR merge, containment, and cleanup complete. |
...
| Exact next sequential-worker action | Complete ES-V01 exact-head hosted validation, review, merge, containment, and cleanup. Do not activate ES-P07, ES-P06 or ES-X01 first. |
...
ES-P05 is terminal and no package is active. The next normal sequential worker must re-reconcile live GitHub and, absent a newer actionable continuation, resume ES-V01. This worker does not begin, prepare, stage or partially implement ES-V01 or any other package.
