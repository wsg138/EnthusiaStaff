...
| Active/selected package | `ES-P07 — Inventory and Ender editing runtime completion`; `ACTIVE`; implementation, required direct wiring coverage and harsh review complete; branch `package/es-p07-inventory-runtime`; ready PR `#112`. |
...
| Exact next action | Finish the latest-handoff checkpoint, take that resulting PR #112 head as immutable, require all exact-head hosted/static/review/Sentinel/canonical Pi evidence, merge normally only if unchanged and fully green, publish terminal/dependency-derived state, and stop. |
...
The original ES-P07 test contract was re-read before merge. Existing direct tests already prove both inventory commands require `enthusiastaff.inventory.view` and the authoritative rank policy separates that read permission from `enthusiastaff.inventory.edit`. A new `InventoryWorkflowWiringTest` closes the remaining direct evidence gap by proving `invsee`/`endersee` share the inventory command/coordinator path, authoritative lookup returns through the viewer entity scheduler, and GUI editing is independently gated by `inventory.edit`.

Intermediate sanity head `321a50f3ca120dba7d7e21542450d4d527dabbd3` passed Java 21 full build/tests, MariaDB/Testcontainers, runtime-JAR/provider-leak inspection, aggregate JaCoCo/Codacy, Wiki validation and Sentinel artifact production. First frozen head `c3545612d370ea237a63394e6a0401edbe650790` also accumulated green exact-head checks, but it is deliberately superseded because the direct wiring test changed tracked content afterward. Neither head may be used as final acceptance.

The first recovery-guard compile attempt failed on incorrect Bukkit package imports for two Paper events and was fixed before the successful sanity build; that failed run remains non-passing evidence. A noncanonical Sentinel comment was also identified as ordinary ignored text after the live operating guide was re-read; final Sentinel evidence must use the exact supported command on the new frozen head.
...
The private LiteBans database remained local. Seven malformed/rejected rows remain a separate later data-policy decision. No production shadow, migration, cutover, authority change, source rewrite or issue #43 activation occurred.
