...
Market API version 1, V025 provider journal/locks/fences, snapshot/checksum lifecycle, blacklist revisions, acquisition/write fencing, exact restoration, and MariaDB race tests are implemented. Staff V19 intent/recovery journal, typed adapter, coordinator, confirmed command surface, review alerts, provider-outage status, migration tests, and exact aggregate import are implemented. Both exact-head Java 21 clean builds pass: Staff has 951 tests with no skips and 192 integration tests; Market has 628 tests with one unrelated skip and all three provider MariaDB tests executed. Local scoped Codacy analysis has zero valid ES-X03 findings, and aggregate/standalone parity is exact at hash `761b6e1e6168782b752cca5bffe6ca8b9330694b38f13b9c19d3a82dbecdaf67` before the paired merges.

## 25. Remaining checklist
Commit this exact validation checkpoint; open the two cross-linked PRs; resolve hosted Codacy, CI, bounded CodeRabbit, and applicable Pi-loader findings; merge normally; prove post-merge parity and containment; remove both temporary branches; publish terminal state.
...
## 27. Final evidence
Starting bases are recorded above. Current standalone head is `daed4d08d96f69f4513431c8bff8b90ada8faa70`; validated Staff source/documentation head is `9d5cf145c09c09f259a05f29b144ed865f5a5a45`. Contract version is 1. Local Market Lizard is 35 versus 36 on its base, with PMD/Opengrep/Trivy at zero. Local Staff package PMD/Opengrep/Trivy are zero; four Lizard findings are unchanged baseline findings in `EnthusiaStaffPaperPlugin` and `MariaDbRuntime`. Staff clean build: 951 tests, zero skipped, including 192 integration tests; exact runtime JAR hashes are recorded in the canonical handoff. Market clean build: 628 tests, one unrelated skip, all three provider MariaDB tests executed; Detekt, coverage, and packaging passed. PR, hosted, Pi, merge, post-merge parity, and branch-cleanup evidence remain pending.
