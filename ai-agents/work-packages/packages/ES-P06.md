...
Production Discord route use or contact; RoseChat PM capture; issue #43; provider-unrelated events.
...
Reconcile producers/store/worker; define event and approved-destination matrices; implement missing rendering/delivery/reload and route validation; inject failure/latency/restart/redirect cases; update docs/state/handoff; review/freeze/exact-head validate/merge/cleanup.

## 13. Acceptance criteria
Every included event is durable and idempotent; retries are bounded; dead letters are inspectable/recoverable; rate/circuit state survives correctly; reload is safe; absent/invalid routes fail explicitly without leaking data. Configuration distinguishes explicitly approved staging and production destinations, but this package contacts only isolated non-production endpoints. Delivery requires HTTPS/TLS. Redirects are rejected by default; any allowed redirect must be revalidated against the approved destination policy without forwarding the webhook secret to a different host or scheme.

## 14. Test requirements
Worker unit/integration tests, MariaDB queue/dead-letter/restart/concurrency tests, duplicate and poison-event cases, configuration reload, redaction, HTTPS enforcement, host/scheme/redirect controls, and isolated fake endpoint tests.

## 15. Static-analysis requirements
Java 21 warnings-as-errors and configured analysis/review bots; zero valid unresolved findings.

## 16. Documentation requirements
Event matrix, approved route configuration, TLS/redirect/privacy controls, retry/dead-letter operations, reload/status, troubleshooting, Wiki, package state/handoff.

## 17. Security and privacy requirements
Environment-backed approved destinations only; HTTPS/TLS required; reject or revalidate redirects without forwarding secrets across host/scheme boundaries; strict redaction/allowlisted fields; no production webhook contact; bounded body/logging.
...
## 27. Final evidence
Unset: event matrix, approved destinations, exact heads, tests/checks, redaction/TLS/redirect review, route-test evidence.
