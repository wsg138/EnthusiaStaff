...
## Resume state
Worker: current owner-directed worker. Branch: `package/es-d16-moderation-read-bridge`. PR: not opened yet. Next action: implement the private read API, real Discord adapters, Worker proxy/session flow, and real-data UI wiring.

## Known blockers
`NONE` at package start. External staging configuration discovered during implementation must remain explicit and must not be bypassed by weakening security.
