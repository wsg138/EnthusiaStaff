...
Last verified: 2026-08-26 through the connected `wsg138` GitHub account for `COMP-ROSECHAT`; other component setup records retain their prior verification state.
...
| `COMP-ROSECHAT` | Enthusia-RoseChat | `components/enthusia-rosechat/` | `wsg138/Enthusia-RoseChat` | `master` | `8fcca5420b0f54207d6efa332327b9fd18edb8d8` | `BLOCKED_LICENSE_REDISTRIBUTION` | Supported communication provider, staff chat, private-message evidence, and presence integration. Verified public fork; checked-in provider license withholds publication/(re)distribution rights required by the public aggregate-copy parity model. |
...
1. Do not invent a repository name or URL. `COMP-ROSECHAT` is resolved to `wsg138/Enthusia-RoseChat`; do not import its source while its verified license/authorization does not permit the required public redistribution.
...
7. Source import is not part of setup. It must occur only in an assigned package after repository, license/history, source SHA, exclusions, and reviewability are verified. License verification is a gate: do not publish or redistribute third-party source beyond verified rights merely to satisfy aggregate parity.
