...
`ES-D16 — Moderation console real-data read bridge` is `BLOCKED` / `PARKED_BLOCKED` in the dedicated Discord lane. PR #187 remains open/unmerged. Frozen executable candidate `83bc4e102b85b9db904e9df4e7f956896fa938bf` repairs the confirmed Paper Flyway classloader defect; validated branch head `587ea47f6e30aa468021497af6bed77d97c2975a` differs afterward only in `moderation-web/README.md`. Exact 587 Coverage `33846514820` / `100939581796` passed clean build/integration tests, 27 provider API source types / zero leaks and 51.97% line / 42.27% branch / 54.27% instruction coverage; staging `33846511302`, web validation `33846514771`, Staff Bot config/artifact `33846514753`/`33846514759`, Sentinel artifact `33846514754`, Codacy zero-annotation static analysis, and manual final-delta review also pass. Live acceptance proved the private browser/Worker/tunnel/StaffBot path reaches Bloom and then exposed an initially empty EnthusiaStaff DB; the owner-authorized transition collector connected successfully but Paper's host context classloader caused Flyway to report `0 migrations`. The exact authority bridge artifact `9926742858` contains V1-V20; contained JAR SHA-256 is `af0e39fa63b84a397efa28fce0160008d4d65562ddb9c0461d00f9d3b5fb5a80`. Current blocker is one owner-operated replacement of only the temporary Paper authority-bridge JAR plus a controlled Paper restart and sanitized migration/collector evidence. Existing `authority.properties` and `collector.properties` stay unchanged; 8771/8766 remain non-public. This parked Discord package does not preempt unrelated dependency-complete universal work.

Canonical D16 blocked handoff: `ai-agents/reports/package-handoffs/2026-09-04-es-d16-paper-migration-classloader-blocked.md`.
