...
- Canonical Pi `35168772060` failed before private dispatch when the public workflow-history lookup returned HTTP 401 `Bad credentials`; no private Pi/Paper/MariaDB runtime ran. A Sentinel status request was posted without a response observed at publication, so durable restart is `NOT RUN`, not pass.
