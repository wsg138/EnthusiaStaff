...
4. Before choosing a `READY` package, reconcile open PRs and branches whose live evidence shows a package is blocked. Exclude an unchanged blocked package even if the default-branch registry is stale, and correct the stale state on the selected package branch.
5. When no continuation work exists, claim the eligible `READY` package with the lowest numerical priority after verifying dependencies, conflicts, duplicate implementation, package accuracy, and required repositories.
6. When no package is eligible, report every incomplete package, its status, blocker or unmet dependency, and the exact condition required to continue; then stop.
