...
4. Before selecting a `READY` package, reconcile any package with an open PR or branch whose live evidence shows it is blocked. Exclude an unchanged blocked package even when the default-branch registry is stale, and correct that stale state on the selected package branch.
5. With no continuation work, select the dependency-complete eligible `READY` package with the lowest numerical priority after verifying no conflict or duplicate implementation exists.
6. When nothing is eligible, report every incomplete package and its exact blocker or unmet dependency, then stop.
