...
Flyway validation runs during `BOOTSTRAP`. A checksum mismatch or unsupported future schema enters `READ_ONLY_FAILURE`; repair is a deliberate operator action, never automatic. Migration imports keep source IDs, initial and last-seen run IDs, checksums, per-table high-water marks, and a durable run manifest. Source active/expiration/removal changes reconcile under a row lock with append-only sanction events. Protected LiteBans history is upsert-idempotent and never stores raw network addresses. Rollback disables the new authority and preserves imported rows for forensic comparison rather than deleting production history.
...
## V14: punishment history and exact sanction lifecycle

`V14__punishment_history_and_exact_sanction_changes.sql` extends append-only `sanction_events` with denormalized case/subject linkage, previous/resulting status and expiration, optional appeal/request IDs and originating runtime. Legacy rows remain valid because linkage columns are nullable and history queries fall back through the sanction relation.

Indexes cover player history, case history, event ordering, request linkage and appeal decisions. One linked website appeal may create at most one reversal event. Exact mutations lock the sanction row and use an expected revision; no in-memory lock is treated as cross-runtime authority. V1–V13 remain byte-identical and Flyway repair is prohibited. Original punishment, request, appeal and event rows are never physically deleted by lifecycle commands.

Before V14 is applied to a production-sized installation, operators must measure row counts and expected index-build time for `cases`, `sanctions`, `sanction_events`, `punishment_requests`, and `punishment_request_events`. Large installations require an approved maintenance window or online schema-change procedure. This feature review does not access production data or run that operational sizing exercise.

## V15: punishment recommendation snapshots

`V15__punishment_recommendation_snapshots.sql` adds nullable `selected_ordinal` and `recommended_sanctions_json` columns to `punishment_steps`. Every new policy-created case writes the configured step ordinal and exact recommended sanction specifications in the same transaction as the case and the sanctions actually applied. The existing `raw_ordinal` and `effective_ordinal` remain unchanged; the selected ordinal is separate because a finite ladder can clamp an out-of-range effective ordinal to its final step.

V14 and older rows remain readable with both new columns null. The application reports those legacy snapshots as unavailable and does not infer a recommendation from applied sanctions because an authorized override may have changed type or duration. Actual sanction type, issue time and expiration remain authoritative and are never recalculated after a ladder edit. Current policies still interpret future recommendations using the current ladder at the calculated ordinal, with out-of-range ordinals selecting the current final step. V1–V14 remain byte-identical and Flyway repair remains prohibited.

## V16: punishment decay eligibility snapshots

`V16__punishment_decay_eligibility_snapshots.sql` adds nullable `decay_eligible` metadata to `punishment_steps`. New policy evaluations store the creating reason policy's explicit decay eligibility in the same transaction as the recommendation, applied sanctions, audit records and outboxes. Related-history loading uses that stored value for each prior offense instead of reinterpreting history through the later reason selected by staff.
